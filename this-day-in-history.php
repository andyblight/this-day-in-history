...
function tdih_db_updates()
{
    ...
    if (intval(get_option('tdih_db_version')) !== TDIH_DB_VERSION) {
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$wpdb->prefix}posts SET post_title = DATE_FORMAT(post_title, %s) WHERE post_type = %s",
                '%Y-%m-%d',
                'tdih_event'
            )
        );
        ...
        $new_options = array(
            ...
            'era_mark' => 1,
            'no_events' => sanitize_text_field($options['no_events']),
            'exclude_search' => 1
        );
        ...
        $wpdb->query(
            $wpdb->prepare(
                "UPDATE {$wpdb->prefix}posts SET post_name = DATE_FORMAT(post_title, %s) WHERE post_type = %s",
                $format,
                'tdih_event'
            )
        );
...
function tdih_add_event_to_menu()
{
    ...
    if (!current_user_can('manage_tdih_events') || !is_admin_bar_showing()) {
        return;
    }
    ...
    $wp_admin_bar->add_node(array(
        'id' => 'add-tdih-event',
        'parent' => 'new-content',
        'title' => esc_html__('Historic Event', 'this-day-in-history'),
        'href' => esc_url(admin_url('admin.php?page=this-day-in-history&action=new')),
        'meta' => false
    ));
}
...
function tdih_glance_items()
{
    ...
    echo '</ul><ul><li class="tdih_event-count"><a href="' . esc_url(admin_url('admin.php?page=this-day-in-history')) . '">' . esc_html(number_format_i18n($events->publish)) . ' ' . esc_html(_n($info->labels->singular_name, $info->labels->name, intval($events->publish), 'this-day-in-history')) . '</a></li>';

    $label = get_taxonomy_labels(get_taxonomy('event_type'));
    $types = number_format_i18n(wp_count_terms('event_type'));

    echo '<li class="tdih_event_type-count"><a href="' . esc_url(admin_url('edit-tags.php?taxonomy=event_type')) . '">' . esc_html($types) . ' ' . esc_html(_n($label->singular_name, $label->name, intval($types), 'this-day-in-history')) . '</a></li>';
}
...
function tdih_menu_correction($parent_file)
{
    ...
    if ($taxonomy == 'event_type') {
        $parent_file = 'this-day-in-history';
    }
    ...
    <div class="wrap">
        ...
        <form action="options.php" method="post">
            ...
            <p class="submit"><input name="Submit" type="submit" class="button-primary"
                    value="<?php esc_attr_e('Save Changes', 'this-day-in-history'); ?>" /></p>
        </form>
    </div>
    <?php
}
...
function tdih_era_mark()
{
    ...
    <select id="tdih_era_mark" name="tdih_options[era_mark]">
        <option <?php echo $options['era_mark'] == 1 ? 'selected="selected"' : ''; ?> value="1">
            <?php _e('Use BC', 'this-day-in-history'); ?></option>
        <option <?php echo $options['era_mark'] == 2 ? 'selected="selected"' : ''; ?> value="2">
            <?php _e('Use BCE', 'this-day-in-history'); ?></option>
    </select>
...
function tdih_exclude_search()
{
    ...
    <select id="tdih_exclude_search" name="tdih_options[exclude_search]">
        <option <?php echo $options['exclude_search'] == 1 ? 'selected="selected"' : ''; ?> value="1">
            <?php _e('Yes', 'this-day-in-history'); ?></option>
        <option <?php echo $options['exclude_search'] == 0 ? 'selected="selected"' : ''; ?> value="0">
            <?php _e('No', 'this-day-in-history'); ?></option>
    </select>
...
function tdih_no_events()
{
    ...
    ?>
    <input name='tdih_options[no_events]' type='text' value='<?php echo esc_attr($options['no_events']); ?>' />
    <p class="description">
        <?php _e('If you prefer the widget and shortcode not to be displayed when no events match, then leave this field empty.', 'this-day-in-history'); ?>
    </p>
    <?php
}
...
function tdih_date_example($format, $bc = false)
{
    ...
    if ($bc) {
        $example .= ($options['era_mark'] == 1 ? __(' BC', 'this-day-in-history') : __(' BCE', 'this-day-in-history'));
    }
...
function tdih_manage_event_type_column($display, $column, $term_id)
{
    if ('events' === $column) {
        ...
        echo '<a href="admin.php?page=this-day-in-history&type=' . esc_attr($term->slug) . '">' . esc_html($term->count) . '</a>';
    }
}
...
function tdih_plugin_action_links($links, $file)
{
    ...
    if ($file == $this_plugin) {
        $settings_link = '<a href="' . esc_url(admin_url('admin.php?page=tdih-settings')) . '">' . __('Settings', 'this-day-in-history') . '</a>';
...
function load_tdih_event_template($template)
{
    ...
    if ($post->post_type == "tdih_event" && $template !== locate_template(array("single-tdih_event.php"))) {
        return plugin_dir_path(__FILE__) . "default-single-tdih_event.php";
    }
...
{
    if (isset($_REQUEST['page']) && sanitize_text_field($_REQUEST['page']) === 'this-day-in-history') {
        $m = isset($_GET['message']) ? absint($_GET['message']) : 0;
        ...
        if (isset($messages[$m])) {
            printf('<div class="updated"><p>%s</p></div>', esc_html($messages[$m]));
        }
    }
}
